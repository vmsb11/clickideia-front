...
function* onAddNewUser({ payload: { user, history } }) {
  ...
  if (response) {
    ...
    yield put(showSuccessAlert('O seu cadastro foi realizado com sucesso, faça seu login', 'Click ideia - Kanban'));
  ...
  //em caso de erro 
    ...
    yield put(showErrorAlert(errorMessage, 'Click ideia - Kanban'));
  }
...
function* onUpdateUser({ payload: { user, token } }) {
  ...
  if (response) {
    ...
    yield put(showSuccessAlert('O seu cadastro foi atualizado com sucesso', 'Click ideia - Kanban'));
