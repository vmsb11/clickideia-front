...
class App extends Component {
  ...
  render() {
    ...
    const { interfaceStore, hideCardPanel, hideErrorAlert, hideSuccessAlert } = this.props;
    const { cardPanel, alerts, modals } = interfaceStore;
    const { showErrors, showSuccess, error, successMessage, title } = alerts;
    
    //renderiza todos os componentes da aplicação
    return (
      <React.Fragment>
        {/** renderiza o painel de cards */}
        <MyModal isOpen={cardPanel.showPanel} isFullScreen={false} icon="bx bx-list-ul" title={cardPanel.title} size="xl" toggle={hideCardPanel}>
          <CardPanel/>
        </MyModal>
