...
class UserForm extends Component {
  ...
  validadeForm(values) {
    ...
    if(!validateMail(mail)) {
      ...
      showToast('info', 'Click ideia - Kanban', 'Email no formato inválido', 'toast-top-center', 5000);
      return false;
    }
    //verifica se o login é válido
    else if(login.length < 6 || login.length > 16) { 
      
      showToast('info', 'Click ideia - Kanban', 'O login deve conter entre 6 e 16 caracteres', 'toast-top-center', 5000);
      return false;
    }
    //verifica se a senha é válida
    else if(password.length < 6 || password.length > 16) {
      
      showToast('info', 'Click ideia - Kanban', 'A senha deve conter entre 6 e 16 caracteres', 'toast-top-center', 5000);
      return false;
    }
